/// @desc DRAW 


// Draw something indicate game has not frozen.
var _x = 128
var _y = 128;
var _i = 0;
var _h = 16;

draw_set_halign(fa_left);
draw_set_valign(fa_top);
draw_text(_x, _y + _h * _i++, $"EXAMPLE 04 - Uses 'unsafe' crawl and finds all .png's");
draw_text(_x, _y + _h * _i++, $"---");
draw_text(_x, _y + _h * _i++, $"Press [ENTER] to give a path to crawl.");
draw_text(_x, _y + _h * _i++, $"---");
draw_text(_x, _y + _h * _i++, $"Time taken  : {(self.timeTaken / 1000)} ms");
draw_text(_x, _y + _h * _i++, $"Found count : {self.foundCount}");
draw_text(_x, _y + _h * _i++, $"Status      : {self.status}");
draw_text(_x, _y + _h * _i++, $"---");


// Draw list of found png's.
draw_text(_x, _y + _h * _i++, $"");
draw_text(_x, _y + _h * _i++, $"Shows first 16 of found PNG's : ");
draw_text(_x, _y + _h * _i++, $"");

var _count = min(16, array_length(self.items));

for(var i = 0; i < _count; i++)
{
  var _item = self.items[i];
  draw_text(_x, _y + _h * _i++, _item.name);
}



